...
export AI_FACTORY_POSTGRES_MIGRATION_URL="postgresql+psycopg://${AI_FACTORY_POSTGRES_USER:-ai_factory}:${AI_FACTORY_POSTGRES_PASSWORD}@${AI_FACTORY_POSTGRES_HOST:-127.0.0.1}:${AI_FACTORY_POSTGRES_PORT:-5432}/${POSTGRES_DATABASE}"
export UV_CACHE_DIR="${UV_CACHE_DIR:-$REPOSITORY_ROOT/agent-service/.uv-cache}"

exec uv run --project "$REPOSITORY_ROOT/agent-service" \
  python -m ai_factory.data_migration "$@"
